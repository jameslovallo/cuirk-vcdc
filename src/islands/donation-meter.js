import { create, css, html } from 'https://unpkg.com/cuick-dev'
...
create('donation-meter', {
	$gross: 0,
	async setup() {
		const data = await fetch(parts.join('')).then((res) => res.json())
		this.$gross.value = data.sales_data_with_null.gross
	},
	template: ({ $gross }) => html`
		<h3>Help us meet our goal of $15,000!</h3>
		<div part="thermometer">
			<div part="track" style=${`--gross: ${$gross.value}`}></div>
		</div>
		<div
